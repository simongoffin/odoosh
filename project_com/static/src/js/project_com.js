odoo.define('project_com.update_kanban', function (require) {
'use strict';
...
KanbanRecord.include({
    ...
    _openRecord: function () {
        if (this.modelName === 'project_com.project_com' && this.$(".o_project_com_kanban_boxes a").length) {
            this.$('.o_project_com_kanban_boxes a').first().click();
        } else {
    ...
    _onKanbanActionClicked: function (ev) {
        var self = this;
        if (this.modelName === 'project_com.task' && $(ev.currentTarget).data('type') === 'set_cover') {
            ev.preventDefault();

            var domain = [['res_model', '=', 'project_com.task'], ['res_id', '=', this.id], ['mimetype', 'ilike', 'image']];
            this._rpc({
                    model: 'ir.attachment',
                    method: 'search_read',
                    domain: domain,
        ...
        function open_cover_images_dialog(attachment_ids) {
            ...
            var $content = $(QWeb.render("project_com.SetCoverModal", {
                cover_id: coverID,
