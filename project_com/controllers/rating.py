...
class RatingProject(http.Controller):
    ...
    @http.route(['/project_com/rating/'], type='http', auth="public", website=True)
    def index(self, **kw):
        project_coms = request.env['project_com.project_com'].sudo().search([('rating_status', '!=', 'no'), ('portal_show_rating', '=', True)])
        values = {'project_coms': project_coms}
        return request.render('project_com.rating_index', values)

    def _calculate_period_partner_stats(self, project_com_id):
        ...
        request.env.cr.execute("""
            ...
            FROM
                rating_rating
            WHERE
                parent_res_model = 'project_com.project_com'
                    AND parent_res_id = %s
                    AND res_model = 'project_com.task'
    ...
    @http.route(['/project_com/rating/<int:project_com_id>'], type='http', auth="public", website=True)
    def page(self, project_com_id=None, **kw):
        user = request.env.user
        project_com = request.env['project_com.project_com'].sudo().browse(project_com_id)
        # to avoid giving any access rights on project_coms to the public user, let's use sudo
        # and check if the user should be able to view the project_com (project_com managers only if it's unpublished or has no rating)
        if not ((project_com.rating_status != 'no') and project_com.portal_show_rating) and not user.sudo(user).has_group('project_com.group_project_com_manager'):
            raise NotFound()

        return request.render('project_com.rating_project_com_rating_page', {
            'project_com': project_com,
            'ratings': request.env['rating.rating'].sudo().search([('consumed', '=', True), ('parent_res_model', '=', 'project_com.project_com'), ('parent_res_id', '=', project_com_id)], order='write_date DESC', limit=50),
            'statistics': self._calculate_period_partner_stats(project_com_id),
        })
